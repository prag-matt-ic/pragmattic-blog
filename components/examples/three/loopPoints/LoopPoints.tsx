...
import LoopParticleSimulationMaterial, {
...
const LoopPointsShaderMaterial = shaderMaterial(
  INITIAL_PARTICLE_UNIFORMS,
  particleVertex,
  particleFragment
);
...
const LoopPoints = forwardRef<Points, Props>(({ isScattered, mesh }) => {
  ...
  const fboCamera = new OrthographicCamera(
    -1,
    1,
    1,
    -1,
  ...
  const simulationShaderMaterial = useRef<
    ShaderMaterial & Partial<SimulationUniforms>
  >(null);
  const pointsShaderMaterial = useRef<
    ShaderMaterial & Partial<PointsShaderUniforms>
  >(null);
      ...
      <points ref={points} dispose={null}>
        <bufferGeometry attach="geometry">
          ...
          <bufferAttribute
            ...
            array={particlesSeeds}
            count={particlesSeeds.length}
            itemSize={1}
          />
        </bufferGeometry>
